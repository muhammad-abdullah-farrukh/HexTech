...
def _slugify(name: str) -> str:
    # Lowercased: URIs are Neo4j MERGE keys, so differently-cased mentions of
    # the same entity must collapse onto one URI. Display casing is preserved
    # separately via the rdfs:label facts_to_graph attaches. Must stay in
    # lockstep with db/kg_staging._slugify (Path A) — Path A/B URIs for the
    # same entity have to be byte-identical or the graph splits per path.
...
def _run_single_chunk(
    ...
    `endpoint` pins every LLM call for this chunk to one server — chunks are
    independent, so run() dispatches them concurrently, one endpoint each. The
    retry loop below stays sequential *within* a chunk (each attempt's feedback
    depends on the previous one), which is why parallelism is at the chunk
    boundary, not here.
    """
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        raw = generate_facts(
            ...
            subject_hint=subject_hint, endpoint=endpoint,
        )
...
def run(session, source_doc, doc_name: str, doc_text: str, qa_pairs: list[dict], ontology: str):
    ...
    def _process_chunk(idx: int, subject: str, chunk_qa: list[dict], endpoint: dict):
    ...
    # Chunks are independent (each produces its own subgraph, merged below) —
    # run them concurrently, one endpoint per chunk round-robin, up to the
    # total configured slot count. Results are collected by index so the merge
    ...
    with ThreadPoolExecutor(max_workers=max(1, min(total_slots, len(chunks)))) as pool:
        futures = {
            pool.submit(
                _process_chunk, idx, subject, chunk_qa,
                LLM_ENDPOINTS[(idx - 1) % len(LLM_ENDPOINTS)],
            ): idx - 1
    ...
    for (subject, _chunk_qa), (g_chunk, accepted, violations, raw) in zip(chunks, chunk_results):
        last_raw = raw
        all_violations.extend(f"[{subject}] {v}" for v in violations)

        if g_chunk is None or len(g_chunk) == 0:
            print(
                f"[Stage 9/10]   ⚠ chunk '{subject}': no valid facts after "
